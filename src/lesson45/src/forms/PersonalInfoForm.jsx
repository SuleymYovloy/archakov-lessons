...
const schema = yup.object({
  ...
  password: yup.string().when('email', {
    ...
    then: (schema) => schema.min(6, 'Пароль должен быть не менее 6 символов').max(32, "Пароль не должен быть больше 32 символов").required("Обязательное поле"),
  }),
});
    ...
    return (
        <div>
            ...
            <div className='row buttons'>
                <Button variant='contained' color='primary'>Очистить</Button>
                <Button onClick={handleSubmit(onSubmit)} variant="contained" color='secondary'>Далее</Button>
            </div>
        </div>
  )
}
